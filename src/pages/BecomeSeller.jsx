...
export default function BecomeSeller() {
  ...
  const OnSubmit = async (e) => {
    ...
    shopData.append("file", avatar);
    shopData.append("shopName", shopName);
    shopData.append("email", email);
    shopData.append("password", pass);
    shopData.append("phoneNumber", phone);
    shopData.append("address", address);
    shopData.append("zipCode", zip);

    // req handle
    axios
      .post(`${baseUrl}/shop/create-shop`, shopData, config)
      .then((res) => {
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 px-4 lg:px-8 gap-4">
      ...
      <div className="mx-auto max-w-md bg-white w-full px-4 lg:px-8 py-6 shadow rounded-lg">
        <form className="space-y-5" onSubmit={OnSubmit}>
          ...
          <div className="flex flex-col gap-1">
            <label className="font-medium text-gray-700 text-lg" htmlFor="ZIP">
              ZIP Code
            </label>
            <input
              className="w-full border border-gray-300 appearance-none rounded-md shadow-sm px-3 py-2 focus:outline-none focus:ring-blue-500 focus:border-blue-500 placeholder-gray-400"
